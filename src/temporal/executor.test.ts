...
describe('TemporalWorkflowExecutor', () => {
  it('pins a run to a versioned Temporal queue and recovers its result', async () => {
    ...
    expect(start.mock.calls[0]?.[1].args[0]).toEqual(expect.objectContaining({ traceId: run.traceId }));
    expect(start).toHaveBeenCalledWith('executeWorkflow', expect.objectContaining({ workflowId: `factory-${run.id}`, taskQueue: 'factory-workflows-v3', searchAttributes: expect.objectContaining({ FactoryId: ['agentic-workflow-factory'], WorkflowVersion: ['3'], Environment: ['local'], Status: ['running'], CorrelationId: [run.traceId], ReleaseBundle: [run.releaseBundleHash], AgentVersions: [JSON.stringify(run.pinnedAgentVersions)] }) }));
