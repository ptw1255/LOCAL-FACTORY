...
export const api = {
  ...
  saveProjectFiles: (projectId: string, files: Array<{ path: string; content: string; expectedSha256?: string | null }>) => request<{ files: ProjectFileRecord[] }>(`/api/projects/${encodeURIComponent(projectId)}/files/batch`, { method: 'PUT', body: JSON.stringify({ files }) }),
  ...
  compileProject: (projectId: string, environment = 'local') => request<ArtifactRecord>(`/api/projects/${encodeURIComponent(projectId)}/compile`, { method: 'POST', body: JSON.stringify({ environment }) }),
