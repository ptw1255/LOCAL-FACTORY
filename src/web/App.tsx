...
import { api } from './api';
import { ErrorState, LoadingState, StatusBadge } from './StatusComponents';
import {
...
import { AppHeader, EmptyState, ProjectSwitcher } from './WorkspaceChrome';
...
let editorLanguageProvidersRegistered = false;

/**
 * Keep editor assistance dependency-free and lazy: the provider only offers
 * fields and IDs from the declarative contract, while server validation remains
 * the source of truth for compilation.
 */
function registerEditorLanguageProviders(monaco: typeof Monaco): void {
  if (editorLanguageProvidersRegistered) return;
  const fieldDocs: Record<string, string> = {
    apiVersion: 'Declarative resource contract version.',
    kind: 'Resource kind, such as Project, Workflow, Agent, Unit, or Deployment.',
    metadata: 'Stable resource identity and descriptive metadata.',
    spec: 'Resource-specific desired configuration.',
    id: 'Stable identifier used by references and telemetry.',
    name: 'Human-readable display name.',
    purpose: 'The bounded outcome this agent box is responsible for.',
    instructions: 'Behavioral instructions supplied to the selected model.',
    skills: 'Named capabilities allowed for this agent box.',
    tools: 'Explicit tool allow-list; undeclared tools are rejected.',
    model: 'Provider and model routing configuration.',
    policies: 'Execution, redaction, approval, and budget boundaries.',
    nodes: 'Workflow work-unit declarations.',
    edges: 'Directed connections between work units.',
  };
  const staticSuggestions = Object.keys(fieldDocs).map((label) => ({
    label,
    kind: monaco.languages.CompletionItemKind.Property,
    insertText: label,
    detail: 'Factory resource field',
    documentation: fieldDocs[label],
  }));
  const provideCompletions = (model: Monaco.editor.ITextModel, position: Monaco.Position) => {
    const word = model.getWordUntilPosition(position);
    const range = { startLineNumber: position.lineNumber, endLineNumber: position.lineNumber, startColumn: word.startColumn, endColumn: word.endColumn };
    const idSuggestions = [
      ...editorLanguageContext.agentIds.map((id) => ({ label: id, kind: monaco.languages.CompletionItemKind.Reference, insertText: id, detail: 'Declared agent box' })),
      ...editorLanguageContext.nodeIds.map((id) => ({ label: id, kind: monaco.languages.CompletionItemKind.Reference, insertText: id, detail: 'Declared work unit' })),
    ];
    return { suggestions: [...staticSuggestions, ...idSuggestions].map((suggestion) => ({ ...suggestion, range })) };
  };
  for (const language of ['yaml', 'json'] as const) {
    monaco.languages.registerCompletionItemProvider(language, { triggerCharacters: [':', '-', '"'], provideCompletionItems: provideCompletions });
    monaco.languages.registerHoverProvider(language, {
      provideHover(model, position) {
        const word = model.getWordAtPosition(position)?.word;
        const description = word === undefined ? undefined : fieldDocs[word];
        return description === undefined ? null : { contents: [{ value: `**${word}**\n\n${description}` }] };
      },
    });
  }
  editorLanguageProvidersRegistered = true;
}
  ...
  return (
    <div className="studio-page">
      ...
      {studioMode === 'canvas' ? <div className="studio-workspace">
        ...
        onReopenGuide={() => {
          window.localStorage.removeItem(`factory.onboarding.${projectId}.dismissed`);
        ...
        onSourceImported={(nextWorkflows, source, artifact) => {
          window.localStorage.setItem(`factory.onboarding.${projectId}.compiled`, 'true');
          setHasCompiled(true);
          if (artifact !== undefined) {
            latestCompiledArtifact.current = artifact;
...
function OperationalTree({
  ...
  onSourceNodeSelect: (nodeId: string) => void;
}) {
  ...
  const sourceCursorListener = useRef<Monaco.IDisposable | null>(null);
  ...
  useEffect(() => () => {
    ...
    sourceCursorListener.current?.dispose();
  ...
  function scheduleSyntaxDiagnostics(nextSource: string, nextPath: string): void {
    ...
    syntaxCheckTimer.current = window.setTimeout(() => {
      ...
      void import('yaml').then(({ parseDocument }) => {
        ...
        const diagnostics = sourceSyntaxDiagnostics(nextSource, nextPath, document.errors);
        setProblems(diagnostics);
        setError(diagnostics[0]?.message ?? null);
      }).catch(() => {
        // The server compiler remains the source of truth if the optional
        // client-side parser cannot be loaded.
      });
    }, 180);
  }
  ...
  async function applyYaml(): Promise<boolean> {
    ...
    try {
      ...
      if (isResourceFile) {
        ...
        onSourceImported(compiled.workflows, source, compiled);
      } else {
...
function RunsView() {
  ...
  useEffect(() => {
    window.sessionStorage.removeItem('factory.observeWorkflowFilter');
    window.sessionStorage.removeItem('factory.observeEnvironmentFilter');
  }, []);
...
function DeploymentsView({ onNavigate }: { onNavigate: (view: ViewId) => void }) {
  ...
  const [artifacts, setArtifacts] = useState<ArtifactRecord[]>([]);
  const [artifactDiff, setArtifactDiff] = useState<{ deploymentId: string; diff: ArtifactDiff } | null>(null);
  ...
  function openObserveDeployment(deployment: DeploymentRecord): void {
  ...
  async function compareArtifacts(deployment: DeploymentRecord, toArtifactId: string): Promise<void> {
    if (toArtifactId === '') return;
    setArtifactDiffLoading(true);
    setError(null);
    try {
      setArtifactDiff({ deploymentId: deployment.id, diff: await api.artifactDiff(projectId, deployment.artifactId, toArtifactId) });
    } catch (diffError) {
      setError(errorText(diffError));
    } finally {
      setArtifactDiffLoading(false);
    }
  }

  function openWorkflowWorkspace(workflowId: string): void {
    sessionStorage.setItem(`${STUDIO_FILE_STORAGE_PREFIX}${projectId}`, `workflows/${workflowId}.workflow.yaml`);
    onNavigate('studio');
  }
  ...
  async function act(deployment: DeploymentRecord, action: DeploymentRecord['history'][number]['action'], artifactId?: string) {
    ...
    try {
      ...
      const promotionApproval = artifactId === undefined
        ? undefined
        : deploymentApprovals.find((approval) => approval.deploymentId === deployment.id && approval.artifactId === artifactId && approval.decision === 'approved');
      const updated = await api.deploymentAction(deployment.id, action, {
        ...(artifactId === undefined ? {} : { artifactId }),
        ...(promotionApproval === undefined ? {} : { approvalId: promotionApproval.id, runId: promotionApproval.runId }),
  ...
  async function decideApproval(deploymentId: string, approval: DeploymentApprovalRecord, action: 'approve' | 'deny'): Promise<void> {
    if (!window.confirm(`${action === 'approve' ? 'Approve' : 'Deny'} deployment promotion for ${approval.artifactId}?`)) return;
      ...
      {deployments.length === 0 ? (
        ...
        <div className="connection-grid">
          {filteredDeployments.map((deployment) => {
            ...
            const healthyArtifacts = new Set(deployment.healthyArtifactIds ?? []);
            const rollbackApprovalArtifacts = approvalArtifacts.filter((artifact) => healthyArtifacts.has(artifact.id));
            const deploymentRuns = runs
              .filter((run) => run.workflowId === deployment.workflowId && (run.environment === undefined || run.environment === deployment.environment))
              .sort((left, right) => right.startedAt.localeCompare(left.startedAt))
              .slice(0, 5);
            const deploymentSignals = deploymentEvidence[deployment.id] ?? [];
            const deploymentLogs = deploymentRunLogs[deployment.id] ?? [];
            const triggerKind = deployment.trigger.trim().toLowerCase();
            const listenerTrigger = triggerKind.includes('schedule') || triggerKind.includes('webhook');
            const triggerLabel = deployment.triggerStatus === 'active' ? 'Listening' : deployment.triggerStatus === 'inactive' ? 'Not listening' : 'Listening status unknown';
            return (
              <article className="connection-card" key={deployment.id}>
                ...
                <dl><div><dt>Desired</dt><dd>{deployment.desiredState}</dd></div><div><dt>Health</dt><dd>{deployment.health}</dd></div><div><dt>Trigger</dt><dd><span>{deployment.trigger}</span>{listenerTrigger ? <span aria-label={`Trigger ${triggerLabel.toLowerCase()}`} className={`trigger-listening trigger-${deployment.triggerStatus}`}>{triggerLabel}</span> : <small>{deployment.triggerStatus}</small>}</dd></div><div><dt>Source</dt><dd><button className="source-link deployment-source-link" onClick={() => openWorkflowWorkspace(deployment.workflowId)} title={'Open workflows/' + deployment.workflowId + '.workflow.yaml'} type="button">workflows/{deployment.workflowId}.workflow.yaml</button></dd></div><div><dt>Updated</dt><dd>{formatDate(deployment.updatedAt)}</dd></div></dl>
