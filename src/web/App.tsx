          ...
          {filteredDeployments.map((deployment) => {
            ...
            return (
              <article className="connection-card" key={deployment.id}>
                ...
                <dl><div><dt>Desired</dt><dd>{deployment.desiredState}</dd></div><div><dt>Health</dt><dd>{deployment.health}</dd></div><div><dt>Trigger</dt><dd>{deployment.trigger} · {deployment.triggerStatus}</dd></div><div><dt>Source</dt><dd><button className="source-link deployment-source-link" onClick={() => openWorkflowWorkspace(deployment.workflowId)} title={'Open workflows/' + deployment.workflowId + '.workflow.yaml'} type="button">workflows/{deployment.workflowId}.workflow.yaml</button></dd></div><div><dt>Updated</dt><dd>{formatDate(deployment.updatedAt)}</dd></div></dl>
